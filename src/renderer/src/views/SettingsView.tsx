...
const SettingsView: React.FC = (): React.ReactElement => {
  ...
  useEffect(() => {
    ...
    const loadKey = async (): Promise<void> => {
      const key = (await window.api.keystore.getFalKey()) as string | null
      if (key) {
        setFalKey(key)
        setIsSaved(true)
      }
  ...
  const handleDeleteKey = async (): Promise<void> => {
    if (
      confirm(
        'Are you sure you want to delete your API key? This will stop all generation features.'
      )
    ) {
        ...
        <div className="flex items-center gap-4 mb-2">
          ...
          <div>
            ...
            <p className="text-subtle-silver text-sm mt-1 font-medium">
              Manage your credentials and local configuration.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="md:col-span-2 space-y-8">
          ...
          <div className="premium-card p-10 space-y-8">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-black text-soft-cloud uppercase tracking-tight">
                  Fal.ai API Configuration
                </h3>
                <p className="text-xs text-subtle-silver mt-1">
                  Required for image and copy generation.
                </p>
              </div>
              <a
          ...
          <div className="p-8 rounded-3xl bg-warm-sunset/5 border border-warm-sunset/20 flex gap-6">
            ...
            <div>
              <h4 className="text-sm font-black text-soft-cloud uppercase tracking-wide mb-1">
                Local-First Architecture
              </h4>
              <p className="text-xs text-subtle-silver leading-relaxed font-medium">
                MonsterCreative is designed to keep your data on your hardware. Your ad copy,
                generated images, and API keys never leave your machine except to communicate
                directly with fal.ai.
              </p>
            </div>
          </div>
        </div>
