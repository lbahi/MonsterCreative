...
const Dashboard: React.FC = (): JSX.Element => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* Welcome Header */}
      <section>
        <h2 className="text-3xl font-black text-soft-cloud tracking-tighter">
          Welcome back, Bahaa
        </h2>
        <p className="text-subtle-silver text-sm mt-1 font-medium">
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {kpis.map((kpi, i) => (
          <div key={i} className="premium-card p-6">
            <p className="text-[10px] uppercase tracking-widest text-subtle-silver font-black mb-3">
              {kpi.label}
            </p>
        ...
        <section className="lg:col-span-6 premium-card p-8">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-5 px-4 mb-2">
              <span className="col-span-2 text-[10px] uppercase tracking-widest text-subtle-silver font-black">
                Campaign Name
              </span>
              <span className="text-[10px] uppercase tracking-widest text-subtle-silver font-black">
                Status
              </span>
              <span className="text-[10px] uppercase tracking-widest text-subtle-silver font-black">
                Platform
              </span>
              <span className="text-[10px] uppercase tracking-widest text-subtle-silver font-black text-right">
                Budget
              </span>
            </div>
            ...
            {campaigns.map((camp, i) => (
              <div
                key={i}
                className="grid grid-cols-5 items-center px-4 py-4 rounded-xl bg-charcoal-surface/30 hover:bg-charcoal-surface border border-white/0 hover:border-white/5 transition-all cursor-pointer group"
              >
                <div className="col-span-2 text-sm font-bold text-soft-cloud group-hover:text-ocean-cerulean transition-colors">
                  {camp.name}
                </div>
                <div>
                  <span
                ...
                <div className="flex gap-1.5">
                  {camp.platform.map((p, pi) => (
                    <span
                      key={pi}
                      className="w-5 h-5 flex items-center justify-center bg-white/5 rounded text-[8px] font-black text-subtle-silver"
                    >
                      {p}
                    </span>
                  ))}
                </div>
                <div className="text-right text-sm font-black text-soft-cloud">{camp.budget}</div>
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="premium-card p-8 flex-1 flex flex-col">
            ...
            <div className="flex-1 space-y-6">
              <div className="p-5 rounded-2xl bg-charcoal-surface border-l-4 border-ocean-cerulean relative overflow-hidden group">
                ...
                <p className="text-[10px] uppercase tracking-widest text-ocean-cerulean font-black mb-3 italic">
                  Best Hook
                </p>
                <p className="text-sm italic font-medium leading-relaxed text-soft-cloud">
                  &quot;Stop wasting thousands on unoptimized ads. MosterAds scales your brand in 24
                  hours.&quot;
                </p>
              </div>

              <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                <p className="text-[10px] uppercase tracking-widest text-subtle-silver font-black mb-1">
                  Impact
                </p>
                <div className="flex items-baseline gap-2">
                  <span className="text-3xl font-black text-warm-sunset tracking-tighter">
                    14.2%
                  </span>
                  <span className="text-[10px] font-bold text-subtle-silver uppercase">
                    Average CTR
                  </span>
                </div>
              </div>
