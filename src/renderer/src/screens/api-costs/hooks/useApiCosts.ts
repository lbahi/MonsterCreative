...
export const useApiCosts = () => {
  ...
  const [modelBreakdown, setModelBreakdown] = useState<
    { model: string; cost: number; quantity: number }[]
  >([])
    ...
    async function fetchData() {
      try {
        ...
        if (
          ...
          !billingResponse.billing_restricted &&
          billingResponse.credits
        ) {
          setBilling({
        ...
        if (usageResponse.time_series) {
          usageResponse.time_series.forEach((bucket: any) => {
            const dateStr = new Date(bucket.bucket).toLocaleDateString('en-US', {
              month: 'short',
              day: 'numeric'
            })
            let bucketTotal = 0
            ...
            bucket.results.forEach((res: any, idx: number) => {
              ...
              const isVideo =
                res.endpoint_id.includes('video') ||
                res.endpoint_id.includes('wan') ||
                res.endpoint_id.includes('kling')
              const isText =
                (res.endpoint_id.includes('text') ||
                  res.endpoint_id.includes('gpt') ||
                  res.endpoint_id.match(/llm|whisper/i)) &&
                !res.endpoint_id.includes('image')
              const type = isVideo ? 'Video' : isText ? 'Text' : 'Image'

              const platform =
                res.endpoint_id.includes('chatgpt') || res.endpoint_id.includes('openai')
                  ? 'OpenAI'
                  : 'fal.ai'
        ...
        if (!usageResponse.error && usageResponse.summary) {
          const breakdown = usageResponse.summary
            .map((row: any) => ({
  ...
  const topModel = useMemo(() => {
    ...
    for (const t of transactions) {
      ...
      if (modelTotals[t.model] > max) {
        max = modelTotals[t.model]
        top = t.model
      }
    }
    return top
