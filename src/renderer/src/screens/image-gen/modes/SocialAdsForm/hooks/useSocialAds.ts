...
export const useSocialAds = (props: SocialAdsFormProps) => {
  const { setGenerating, setGeneratedImages, setGenerated, refImage, setRefImage, model } = props
  ...
  const generateSocialAd = async (templateOverride?: Template) => {
    ...
    try {
      ...
      const posterDirective =
        'This is a poster design that should be engaging for social media, showing some of the best features of the product with high energy, bold visual impact, and eye-catching composition.'
      ...
      const result = await window.api.fal.nanoBananaEdit({
        model,
        prompt: finalPrompt,
  ...
  const openOutputFolder = () => window.api.social.openOutputFolder()
