...
function LogoTikTok(): React.ReactElement {
  return (
    <svg viewBox="0 0 24 24" width="16" height="16" fill="none">
      <rect width="24" height="24" rx="4" fill="#010101" />
      <path
        d="M16.6 7.3c-.9-.6-1.5-1.6-1.6-2.8H12.8v10c-.1.9-.8 1.6-1.8 1.6-1 0-1.8-.8-1.8-1.8s.8-1.8 1.8-1.8c.2 0 .4 0 .6.1v-2.4c-.2 0-.4-.1-.6-.1-2.3 0-4.1 1.8-4.1 4.1s1.8 4.1 4.1 4.1 4.1-1.8 4.1-4.1V9.5c.8.6 1.8.9 2.8.9V8.2c-.5 0-1-.3-1.3-.9z"
        fill="white"
      />
    </svg>
  )
}
function LogoFacebook(): React.ReactElement {
  return (
    <svg viewBox="0 0 24 24" width="16" height="16" fill="#1877F2">
      <path d="M24 12.073C24 5.405 18.627 0 12 0 5.373 0 0 5.405 0 12.073c0 6.028 4.388 11.024 10.125 11.926V15.55H7.078v-3.477h3.047v-2.65c0-3.023 1.79-4.697 4.533-4.697 1.312 0 2.686.235 2.686.235v2.967H15.83c-1.491 0-1.956.93-1.956 1.885v2.26h3.328l-.532 3.477h-2.796v8.449C19.612 23.097 24 18.1 24 12.073z" />
    </svg>
  )
}
function LogoGoogle(): React.ReactElement {
  return (
    <svg viewBox="0 0 24 24" width="16" height="16" fill="none">
      <path
...
function LogoPinterest(): React.ReactElement {
  return (
...
function LogoTwitter(): React.ReactElement {
  return (
    <svg viewBox="0 0 24 24" width="16" height="16" fill="white">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.728-8.835L1.254 2.25H8.08l4.259 5.63 5.905-5.63zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  )
}
function LogoWooCommerce(): React.ReactElement {
  return (
    <svg viewBox="0 0 24 24" width="16" height="16">
      <rect width="24" height="24" rx="4" fill="#7F54B3" />
      <text x="3" y="16" fontSize="8" fontWeight="800" fill="white" fontFamily="sans-serif">
        Woo
      </text>
    </svg>
  )
...
/* ─── Ratio preview box ───────────────────────────────────────────────────── */
function RatioBox({ w, h }: { w: number; h: number }): React.ReactElement {
  const maxW = 22
  const maxH = 28
  const ratio = w / h
  ...
  if (ratio >= 1) {
    pw = maxW
    ph = Math.max(8, Math.round(maxW / ratio))
  } else {
    ph = maxH
    pw = Math.max(8, Math.round(maxH * ratio))
  }
  return (
    <div
      style={{
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <span
            style={{
              fontSize: 11,
              fontWeight: 600,
              color: selected ? 'var(--ma-accent-light)' : 'rgba(255,255,255,0.8)',
              overflow: 'hidden',
        ...
        {!isCustom && (
          <span
            style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', fontFamily: 'var(--font-mono)' }}
          >
            {fmt.w}×{fmt.h}
          </span>
        )}
...
function NumBadge({ n }: { n: number }): React.ReactElement {
  return (
    <div
      style={{
        width: 20,
...
export function ResizeForm({
  ...
  generating,
  totalCost
}: ResizeFormProps): React.ReactElement {
  ...
  return (
    <div
      style={{ display: 'flex', flexDirection: 'column', gap: 10, fontFamily: 'var(--font-body)' }}
    >
      <input
        ref={fileInputRef}
        type="file"
        ...
        <div style={cardStyle}>
          ...
          {sourceFile && previewUrl ? (
            ...
            <div
              ...
              style={{ ...dashedBox, cursor: 'pointer' }}
            >
          ...
          <div style={dashedBox}>
            ...
            >
              ...
              <div style={infoRow}>
                ...
                >
                  {modelDef?.endpoint.split('/').slice(-2).join('/')}
                </span>
              </div>
            </div>
          </div>
              ...
              <div style={{ width: '100%' }}>
                ...
                <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  ...
                  {selectedFormats.length > 6 && (
                    <span
                      style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', padding: '2px 4px' }}
                    >
                      +{selectedFormats.length - 6} more
                    </span>
                  )}
                </div>
              </div>
      ...
      >
        ...
        >
          ...
          <button
            onClick={() =>
              setSelectedFormats(
                selectedFormats.length === PLATFORM_FORMATS.length
                  ? []
                  : PLATFORM_FORMATS.map((f) => f.id)
              )
            }
            style={{
              fontSize: 10,
      ...
      <button
        ...
        style={{
          ...
          background: isDisabled
            ...
            : 'linear-gradient(135deg, var(--ma-accent) 0%, #8B5CF6 100%)',
      ...
      >
        {generating ? (
          <>
            <RefreshCw size={15} style={{ animation: 'spin 1s linear infinite' }} /> Resizing
            Formats...
          </>
        ) : !sourceFile ? (
          <>
          ...
          <>
            <Wand2 size={15} /> Resize {selectedFormats.length} Format
