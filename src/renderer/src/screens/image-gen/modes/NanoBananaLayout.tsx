...
export function NanoBananaLayout({
  ...
  const handleMultipleImageUpload = async (event: ChangeEvent<HTMLInputElement>): Promise<void> => {
  ...
  const handleMultipleDrop = async (event: DragEvent<HTMLDivElement>): Promise<void> => {
    event.preventDefault()
    setDragAsset(false)
  ...
  const removeAssetImage = (indexToRemove: number): void => {
    setAssetImages(assetImages.filter((_, i) => i !== indexToRemove))
  }

  const uploadBoxStyle = (isDragging: boolean, hasImage: boolean): React.CSSProperties => ({
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      >
        ...
        >
          ...
          <label
            ...
            onDrop={(event) =>
              handleDrop(event, setRefImage as (value: string) => void, setDragRef)
            }
          >
            <input
              type="file"
              accept="image/*"
              style={{ display: 'none' }}
              onChange={(event) => handleImageUpload(event, setRefImage as (value: string) => void)}
            />
            {refImage ? (
              <>
                <img
              ...
              <>
                ...
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', textAlign: 'center' }}>
                  Recreate The same design in 1s
                </div>
              </>
            )}
          </label>
          ...
          >
            {assetImages.length > 0 ? (
              ...
              >
                ...
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', textAlign: 'center' }}>
                  Faces, logos or products (up to 9)
                </div>
              </label>
            )}
          </div>
      ...
      <div style={{ background: 'translate', display: 'flex', gap: 12, padding: '10px 0' }}>
        <div style={{ flex: 1 }}>
          ...
          <select
            value={nbModel}
            onChange={(event) => setNbModel(event.target.value)}
            style={{
              width: '100%',
              padding: '12px',
      ...
      >
        ...
        >
          ...
          <ChevronDown
            ...
            style={{ transform: advOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: '0.2s' }}
          />
        </button>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <input
                  ...
                  max={4}
                  value={numOutputs}
                  onChange={(event) => setNumOutputs(Number(event.target.value))}
                  style={{ width: '100%', accentColor: 'var(--ma-accent)' }}
                />
              </div>
              <div>
                <label
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                <input
                  type="checkbox"
                  checked={searchGround}
                  onChange={(event) => setSearchGround(event.target.checked)}
                  style={{ width: 16, height: 16, accentColor: 'var(--ma-accent)' }}
                />
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                <input
                  type="checkbox"
                  checked={limitGen}
                  onChange={(event) => setLimitGen(event.target.checked)}
                  style={{ width: 16, height: 16, accentColor: 'var(--ma-accent)' }}
                />
              ...
              <div>
                ...
                <select
                  value={thinking}
                  onChange={(event) => setThinking(event.target.value as NanoBananaThinkingLevel)}
                  style={{
                    width: '100%',
                    padding: '8px 12px',
                    background: 'rgba(255,255,255,0.04)',
                ...
                >
                  <option value="minimal" style={{ background: '#111124' }}>
                    Minimal
                  </option>
                  <option value="high" style={{ background: '#111124' }}>
                    High
                  </option>
                </select>
              </div>
      ...
      <button
        ...
        style={{
          ...
          gap: 10,
          boxShadow:
            generating || !prompt || !refImage ? 'none' : '0 4px 20px rgba(108,99,255,0.4)',
          transition: 'all 0.2s',
          marginTop: 10
        }}
      >
