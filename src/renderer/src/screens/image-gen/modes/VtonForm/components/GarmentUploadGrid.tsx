...
export function GarmentUploadGrid(props: GarmentUploadGridProps) {
  const { garmentSlots, draggingSlot, setDraggingSlot, fileInputRefs, onDrop, onFileSelect, onClear, onClearAll, filledCount } = props;

  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <label style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.8px', fontWeight: 600 }}>
          <Shirt size={12} style={{ verticalAlign: '-1px', marginRight: 4 }} />
          1 · Upload Garments ({filledCount}/6)
        </label>
        {filledCount > 0 && (
          <button onClick={onClearAll} style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>
            Clear All
          </button>
        )}
      </div>
        ...
        {garmentSlots.map((slot, idx) => {
          ...
          return (
            <div key={slot.id} style={{ position: 'relative' }}>
              ...
              />
              <div
                onDragOver={e => { e.preventDefault(); setDraggingSlot(slot.id); }}
                ...
                style={{
                  ...
                  background: slot.image ? '#000' : isDragging ? 'rgba(108,99,255,0.05)' : 'rgba(255,255,255,0.02)',
              ...
              >
                {slot.image ? (
                  <>
                    <img src={slot.image} alt={slot.label} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                    <button
                  ...
                  <>
                    <Upload size={16} color={isDragging ? 'var(--ma-accent)' : 'rgba(255,255,255,0.15)'} style={{ marginBottom: 4 }} />
                    <p style={{ fontSize: 9, color: isRequired ? 'rgba(108,99,255,0.6)' : 'rgba(255,255,255,0.25)', margin: 0, textAlign: 'center', padding: '0 4px', lineHeight: 1.3 }}>
                      {slot.label}
                    </p>
                    {isRequired && (
                      <span style={{ fontSize: 8, color: 'var(--ma-accent)', marginTop: 2, fontWeight: 700 }}>REQUIRED</span>
                    )}
                  </>
                )}
              </div>
