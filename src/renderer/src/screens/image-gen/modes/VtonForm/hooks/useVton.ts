...
export const useVton = (props: VtonFormProps) => {
  ...
  const setSlotImage = useCallback((slotId: number, image: string | null) => {
  ...
  const processFile = useCallback((file: File, slotId: number) => {
  ...
  const handleSlotFileSelect = useCallback((e: React.ChangeEvent<HTMLInputElement>, slotId: number) => {
    if (e.target.files?.[0]) {
  ...
  const clearSlot = useCallback((e: React.MouseEvent, slotId: number) => {
    e.stopPropagation();
