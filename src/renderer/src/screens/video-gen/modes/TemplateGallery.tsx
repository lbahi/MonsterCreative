...
interface TemplateGalleryProps {
  onSelectTemplate: (template: VideoTemplate, config: { model: string, duration: number, aspectRatio: string }) => void;
  disabled?: boolean;
}
...
export function TemplateGallery({ onSelectTemplate, disabled }: TemplateGalleryProps) {
  ...
  const handleGenerateConfirm = () => {
    if (previewTemplate) {
      onSelectTemplate(previewTemplate, { model: targetModel, duration: targetDuration, aspectRatio: targetAspect });
      ...
      {previewTemplate && (
        <div style={{
          ...
          <button 
            ...
            style={{ position: 'absolute', top: 32, right: 32, zIndex: 10, background: 'rgba(255,255,255,0.2)', border: '1px solid rgba(255,255,255,0.4)', color: '#FFF', width: 40, height: 40, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.2s' }}
          ...
          <div style={{
            ...
            width: '100%', maxWidth: 900, display: 'flex', overflow: 'hidden', boxShadow: '0 24px 60px rgba(0,0,0,0.6)'
          }}>
            
            {/* Left Box: Video Preview */}
            <div style={{ flex: '1.2', background: '#f8f8f8', minHeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
              <div style={{ width: '100%', height: '100%', borderRadius: 8, overflow: 'hidden', position: 'relative', background: '#000' }}>
                {previewTemplate.previewVideo ? (
                  <video src={encodeURI(previewTemplate.previewVideo)} autoPlay loop muted playsInline style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
                  <img src={previewTemplate.coverImage} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                )}
              </div>
            </div>

            {/* Right Box: Details */}
            <div style={{ flex: '1', padding: '32px', display: 'flex', flexDirection: 'column', color: '#1A1A1A' }}>
              <div style={{ marginBottom: 16 }}>
                <h2 style={{ fontSize: 12, fontWeight: 800, color: '#7C3AED', margin: '0 0 4px 0', textTransform: 'uppercase', letterSpacing: '0.8px' }}>
                  AI Video
                </h2>
              ...
              <div style={{ background: '#F5F3FF', borderRadius: 8, padding: 16, marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 24 }}>
                <div style={{ display: 'flex', gap: 16, alignItems: 'center', color: '#666', fontSize: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', margin: '4px 0 0 0' }}>
            Click a style to instantly orchestrate the optimal AI settings and animate the source image.
          </p>
        </div>
      </div>
          ...
          >
            ...
            <div style={{ width: '100%', height: 160, position: 'relative', background: '#000' }}>
              ...
              <div style={{ 
                ...
                <span style={{ fontSize: 12, fontWeight: 600, color: '#FFF' }}>{template.label}</span>
              </div>
              ...
              >
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--ma-accent)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {/* Meta */}
            <div style={{ padding: '8px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--ma-border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, color: 'rgba(255,255,255,0.4)' }}>
                <Clock size={10} />
                <span style={{ fontSize: 10, fontWeight: 500 }}>{template.recommendedDuration}s</span>
              </div>
              
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, color: 'var(--ma-green)' }}>
                <Zap size={10} />
                <span style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase' }}>1-Click</span>
              </div>
            </div>
