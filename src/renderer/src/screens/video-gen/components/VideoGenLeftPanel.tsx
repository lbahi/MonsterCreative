  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      >
        ...
        <label
          style={{
            border: `2px ${sourceImage ? 'solid' : 'dashed'} ${sourceImage ? 'var(--ma-green)' : 'rgba(255,255,255,0.12)'}`,
            borderRadius: 12,
            padding: 30,
            textAlign: 'center',
            cursor: 'pointer',
            background: sourceImage ? 'rgba(34,197,94,0.03)' : 'rgba(255,255,255,0.02)',
            height: 280,
            display: 'flex',
      ...
      {activeMode === 'manual' && modelId === 'fal-ai/kling-video/v2.6/pro/image-to-video' && (
        ...
        >
          ...
          <label
            style={{
              border: `2px ${endImage ? 'solid' : 'dashed'} ${endImage ? 'var(--ma-accent)' : 'rgba(255,255,255,0.12)'}`,
              borderRadius: 12,
              padding: 30,
              textAlign: 'center',
              cursor: 'pointer',
              background: endImage ? 'rgba(108,99,255,0.03)' : 'rgba(255,255,255,0.02)',
              height: 160,
              display: 'flex',
              flexDirection: 'column',
              alignItems: 'center',
          ...
          >
            <input
              ...
              accept="image/*"
              style={{ display: 'none' }}
              onChange={(e) => handleImageUpload(e, setEndImage)}
              disabled={generating}
            />
            {endImage ? (
              <>
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              ...
              >
                {isFixedDuration ? (
                  <option value={selectedModelInfo.fixedDuration ?? 0}>
                    {selectedModelInfo.fixedDuration}s
                  </option>
                ) : (
                  VIDEO_DURATIONS.map((d) => (
                    <option
                      key={d}
                      value={d}
                      disabled={!selectedModelInfo.supportedDurations.includes(d)}
                      style={{ background: '#111' }}
                    >
                      {d}s{' '}
                      {!selectedModelInfo.supportedDurations.includes(d) ? '(Unsupported)' : ''}
                    </option>
                  ))
                )}
              </select>
              ...
              {isFixedDuration && selectedModelInfo.fixedDurationNote && (
                ...
                >
                  <span>⚠️</span>
                  <span>
                    {selectedModelInfo.fixedDurationNote}. Duration selection is disabled for this
                    model.
                  </span>
                </div>
              )}
            ...
            <div style={{ gridColumn: 'span 2' }}>
              ...
              >
                {VIDEO_RESOLUTIONS.map((res) => (
                  <option key={res} value={res} style={{ background: '#111' }}>
                    {res}
                  </option>
                ))}
              </select>
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                height: 20,
                borderRadius: 10,
                background: audio ? 'var(--ma-green)' : 'rgba(255,255,255,0.1)',
                border: 'none',
                position: 'relative',
                cursor: generating || !selectedModelInfo?.supportsAudio ? 'not-allowed' : 'pointer',
                opacity: !selectedModelInfo?.supportsAudio ? 0.3 : 1,
